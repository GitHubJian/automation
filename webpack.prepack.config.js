...
const PACK_PATH = path.join(__dirname, '.temp/vue-webpack');
const VUE_PATH = path.join(__dirname, 'vue');
const TEMP_PATH = path.join(__dirname, 'build');

const vueTmp = fs.readFileSync(
  path.join(TEMP_PATH, 'vue.template.js'),
  'utf-8'
);
...
pageList.forEach(v => {
  ...
  fs.writeFileSync(
    `${entryPath}/index.js`,
    vueTmp.replace('{{PATH}}', path.join(VUE_PATH, outputPath, 'index.vue'))
  );
});
